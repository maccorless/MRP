    ...
    <div className="space-y-6">
      ...
      <fieldset>
        <legend className={LABEL}>
          {t("accred.categories.legend")} <span className="text-red-500" aria-hidden="true">*</span>
          <span className="sr-only">{t("accred.category.required")}</span>
        </legend>
        <p className={`${HELP} mb-3`}>{t("accred.categories.help")}</p>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center">
                      ...
                      <InfoTooltip text={cat.helpText} ariaLabel={t("accred.tooltip.ariaLabel")} />
                    </div>
                ...
                {isChecked && (
                  <div className="mt-3 ml-7" onClick={(e) => e.preventDefault()}>
                    ...
                    >
                      {t("accred.quantity.label").replace("{cat}", cat.shortLabel)}{" "}
                    ...
                    {orgType !== "enr" && quantities[cat.value] > 100 && (
                      <p className="text-xs text-red-600 mt-0.5">{t("accred.quantity.max100")}</p>
                    )}
                  </div>
                )}
              </label>
        ...
        {(errors?.category || !anySelected) && (
          <p id="err-category" className="mt-2 text-xs text-red-500" role="alert">
            {errors?.category ?? t("accred.categoryError")}
          </p>
        )}
      </fieldset>
      ...
      {needsSportPicker && (
        <div>
          <label htmlFor="sports_specific_sport" className={LABEL}>
            {t("accred.sportPicker.label")}{" "}
            <span className="text-red-500" aria-hidden="true">*</span>
            <span className="sr-only">{t("accred.sportPicker.required")}</span>
          </label>
          <p className={`${HELP} mb-1`}>
            {t("accred.sportPicker.help")}
          </p>
          <select
            ...
            className={INPUT}
          >
            <option value="">{t("accred.sportPicker.placeholder")}</option>
      ...
      <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 text-xs text-gray-500">
        <span className="font-medium text-gray-700">{t("accred.nocE.heading")}</span>
        {t("accred.nocE.body")}
      </div>
      ...
      <div>
        <label htmlFor="about" className={LABEL}>
          {t("accred.about.label")} <span className="text-red-500">*</span>
        </label>
        <textarea
          id="about"
          name="about"
          ...
          placeholder={t("accred.about.placeholder")}
      ...
      {orgType === "enr" && (
        <div className="mt-4">
          <label htmlFor="enr-programming-type" className="block text-xs font-medium text-gray-700 mb-1">
            {t("accred.enrType.label")} <span className="text-red-500">*</span>
          </label>
          <textarea
