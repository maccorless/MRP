...
**Branch:** All work must be done on the `agent` branch (`git checkout agent` before starting). Phases 1–3 (API key auth, internal service layer, MCP server, OpenAPI spec) are already committed there.
...
In `src/db/schema.ts`, find `auditActionEnum`. The enum currently ends with `"excel_reimport"`. Add after it:
...
Expected: import error — `actions.ts` doesn't exist yet.
...
The modal handles the creation form and displays the raw key exactly once after creation.
...
The server component queries all keys joined to admin users, ordered newest first. The revoke button is an inline form (no client component needed).
...
In `src/app/admin/ioc/nav.tsx`, find the `NAV` array. It currently ends with:
...
Click "API Keys" → "Generate API Key". Enter any admin user email and a label (e.g. "Claude Desktop"). Submit. Confirm the raw key is displayed in the modal in `prp_` format. Click "Done — I've copied the key".

- [ ] **Step 7.4: Verify the key appears in the table**

Confirm the new row shows the label, user email, key prefix (`prp_…`), and "Active" status.

- [ ] **Step 7.5: Revoke the key**

Click "Revoke". Confirm the confirmation dialog fires, the page reloads with a green success banner, and the row shows "Revoked" in red.

- [ ] **Step 7.6: Verify the key authenticates the MCP endpoint**

Generate a fresh key. Run:

```bash
curl -X POST http://localhost:3000/api/mcp \
  -H "Authorization: Bearer <paste raw key here>" \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"tools/list","params":{}}'
```

Expected: a JSON-RPC response with an array of 11 tool names.

Revoke the key. Re-run the same curl. Expected: `{"error":"Invalid or expired API key"}` with HTTP 401.
...
Expected: all tests pass. `agent-permissions.test.ts` (11 tests) and `agent-api-key-actions.test.ts` (5 tests) both green.
...
- API Keys page (table + status badges + banners): Task 5 ✓
- Nav item: Task 6 ✓
- Browser smoke test including curl auth verification: Task 7 ✓

**Out of scope (by design):** Rate limiting (noted as stretch in the architecture doc). Merge to main — the design plan specifies the agent branch stays separate until the full MCP surface is verified end-to-end; that decision belongs to the human.
