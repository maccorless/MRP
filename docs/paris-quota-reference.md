...
This means: the IOC tracks the **full allocation history inline** in the same row, not in separate log tables. Our PRP schema needs an `allocation_revisions` timeline to mirror this — overwriting an allocation would destroy audit trail the IOC expects to keep.
...
So ~67% of the press accreditation pool is NOC-allocated. ~33% is non-NOC (agencies, IOC direct, IFs, sport specialists, ENRs, contingency) — all of which PRP currently models thinly or not at all.
...
## Findings that matter for PRP design

1. **Nine categories, not one bucket.** The PRP schema must carry per-category slots: NOC E, NOC Es, E, Es, EP, Eps, ET, EC, ENR — plus venue-scoped Ex/EPx for co-host cities (100 total in Paris's Tab 8). ✅ Our schema already has most of these; verify Ex/EPx is there.
2. **Allocation is versioned.** Paris stored 5 side-by-side snapshots with per-block comments. PRP must log allocations as revisions, not overwrite. Confirm our audit log captures enough to reconstruct any prior version.
...
8. **No rights-holding broadcasters (RHBs)** in this workbook at all — those are tracked separately from press quota. PRP is press-only, which aligns.

---

## Ground-truth numbers for PRP quota tests
...
**Implication for PbN Excel question**: "Excel import/sync" likely means: PRP needs to produce a spreadsheet in **this exact shape** on export, and potentially accept one on import.
