...
- **Contact info editability (A14)** — Authorized users (NOC admin, OCOG admin) should be able to edit contact info fields on a submitted EoI. Design decision required: when the contact email changes, does the magic link stay tied to the original email (Option A), or is a new magic link issued to the new email (Option B)? Implementation blocked pending this decision.
